...
Thalamus is the IDS. Amygdala is the fingerprint-blocking firewall. Hillock is the microsegmentation enforcer. All three are capabilities of the same Synapse binary, now deployed at an inside chokepoint rather than at the edge: the *Hauptkampffeld* is the same agent in a different role.
...
**Hillock enforces east-west.** Hillock is not just an edge firewall. The same policy engine that runs at the edge runs on every Synapse-deployed host. Per-workload allowlists. Per-process egress policy. Per-service ingress. The policy is centrally authored, distributed to every Synapse, and enforced in eBPF. When a workload tries to talk to another workload it has no business reaching, the packet drops in-kernel, and Thalamus sees the attempt as telemetry.
...
**Cerebellum feeds the rule engine.** New IOCs from Layer 01, new behavioural clusters from production telemetry, local findings shipped up by Cortex on each sensor: Cerebellum compiles them into Thalamus rules and Hillock policy and pushes them to every Synapse in seconds. The rate of policy refresh is bounded by Cerebellum's inference cycle, not by a release process.
