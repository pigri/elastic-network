...
The word doing the work there is *independent*. Corroboration is only as strong as the sources are uncorrelated, and most of the CTI market is not. A large share of commercial feeds resell, relabel, or quietly scrape the same handful of upstream lists, so "fourteen feeds agree" can really mean "one list, laundered fourteen times." Counting agreements is worthless when the agreements are copies. The staff knew this instinctively: ten men repeating the same rumour from the same *estaminet* is one report, not ten; five men who each saw the column from a different hill is corroboration. Confidence comes from *diversity of observation*, not volume of it.

So a useful Layer 01 weights its sources by independence and by mode. A commercial feed, your own passive DNS, a canary you planted, a peer in your ISAC, and your own sensor telemetry are five genuinely different ways of seeing the same indicator — and when they converge, the verdict is strong precisely because no two of them could have copied the others. A confidence score that treats every source as equal can be gamed by whoever poisons the cheapest feed. One that understands which sources are independent is much harder to fool.
...
The honeypot's most important job, though, is not catching the one intruder who tripped it — it is **collecting fingerprints that make detection better everywhere else**. Every interaction yields the attacker's JA4+ fingerprints and tooling, not merely the fact of the touch. Cerebellum turns what a single decoy harvested into a detection signature and a scored verdict, and pushes both to the whole fleet: from that moment, every other Synapse *recognises that adversary's stack on sight* — Thalamus detects it, Amygdala blocks it — without that customer ever having been attacked. One honeypot interaction upgrades detection for everyone. That is the *Scheinbatterie* that revealed the enemy's guns, at fleet scale: the decoy is a sensor whose product is better detection, not just a tripwire whose product is an alert.
...
Underneath the card it is plain JSON, designed to be consumed by Amygdala, Thalamus, and Workflow without a human in the path.
